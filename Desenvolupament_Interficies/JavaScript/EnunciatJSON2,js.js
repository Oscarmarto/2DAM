...
const llistat_Id_Nom_Email_reduce = () => {
    let llistat = dadesUsuaris.reduce((acum, elem) => {
        acum.push({
            id: elem.id,
            name: elem.name,
            email: elem.email
...
const llistaAmbAddress = () => {
        ...
        return {
            ...
            name: element.name,
            email: element.email,
            adress: element.address.street+"  "+element.address.suite+"   "+element.address.zipcode
        }
    }
    )
    console.log(llistat)
...
const llistaIds_EmpresesSenseWebsite = () => {
    return dadesUsuaris.filter(element => element.website === "").map(element => {return{id: element.id}})
}
