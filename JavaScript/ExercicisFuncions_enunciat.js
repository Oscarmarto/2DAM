...
const comptaCaractersDia = (arrayDies) => 
    arrayDies.map(dia => dia.length);

const numCaracters = comptaCaractersDia(dies);
dies.forEach((element,pos)=>console.log(element+":"+numCaracters[pos]));

// dies.forEach(dia => console.log(dia, dia.length));


//Mostra els dies que ténen més de 7 caracters (filter)
console.log("----------------------------------");

const diesMajor7caracters = dies.filter((unDia) => unDia.length>7);
console.log(diesMajor7caracters);

console.log("----------------------------------");
/* Ara volem fer el mateix, però que ens torne un String amb els dies que ténen més de 7 caracters */

let megaString="";
diesMajor7caracters.forEach(unELement => megaString+unELement);
